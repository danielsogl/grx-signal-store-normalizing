...
import { NormalizedBlogStore, provideNormalizedBlogStore } from '../../store/normalized-blog.store';
...
@Component({
  ...
  providers: [provideNormalizedBlogStore()],
  template: `
    <div class="container">
      ...
      <div class="section" *ngIf="selectedPost">
        ...
        <div class="form-group">
          ...
          <textarea id="comment" [(ngModel)]="newComment.comment" rows="3"></textarea>
        </div>
...
export class NormalizedBlogDemoComponent implements OnInit {
  ...
  get users(): User[] {
    return this.store['usersList']?.() as User[] || [];
  }
  ...
  get comments(): Comment[] {
    return this.store['commentsList']?.() as Comment[] || [];
  }
  ...
  submitComment(): void {
    if (!this.selectedPost || !this.newComment.author || !this.newComment.comment) return;
